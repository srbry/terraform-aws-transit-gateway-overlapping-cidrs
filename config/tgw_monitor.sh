...
`sudo tc qdisc add dev eth0 root handle 1: htb > /dev/null 2>&1`
`sudo tc qdisc add dev eth0 ingress > /dev/null 2>&1`
        ...
        if [ "${tcIp[$m]}" != "${func_rtIp[$m]}" ]; then
         ...
         # Remove filters
          ...
          tempHandleIngress="$(tc -p filter show dev eth0 root | grep -B 2 $tempHandleVpcIp | grep -B 1 "${tcIp[$m]}" | awk -F'800::' '{print $2}' | awk {'print $1}' | tr -d '\040\011\012\015')"
           `sudo tc filter del dev eth0 parent ffff: protocol ip prio 1 handle 800::$tempHandleIngress u32`
           ...
           #Egress
            ...
            tempHandleEgress="$(tc -p filter show dev eth0 | grep -B 2 $tempHandleNatIp | grep -B 1 "$m" | awk -F'800::' '{print $2}' | awk {'print $1}' | tr -d '\040\011\012\015')"
             `sudo tc filter del dev eth0 parent 1: protocol ip prio 1 handle 800::$tempHandleEgress u32`
             if [ $PRIMARY_NAT == $MY_INSTANCE_ID ]; then
...
for k in "${!func_rtIp[@]}"; do
    ...
    if [[ -z ${!tcIp[*]} && -n ${!func_rtIp[*]} ]]; then
        ...
        `sudo tc filter add dev eth0 parent ffff: protocol ip prio 1 u32 match ip dst ${func_rtIp[$k]} action nat ingress ${func_rtIp[$k]} $k`
        `sudo tc filter add dev eth0 parent 1: protocol ip prio 1 u32 match ip src $k action nat egress $k ${func_rtIp[$k]}`
...
while [ . ]; do
 ...
 while [ . ]; do
        ...
        if  [[ (-z $VPC_IP && -z $NAT_IP) && $i == 0 ]] ; then ## if key and value and 0 and this is 1st loop
                ...
                `sudo tc filter del dev eth0 parent ffff:`
